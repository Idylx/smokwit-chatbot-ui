...
def run():
    # assistant session state key to be smoker-specific
    ...
    st.set_page_config(page_title="Mr Doe", layout="centered")
    st.title(f"Mr Doe")
    st.write("Peu importe où tu en es, t’es pas seul ! J'essaie aussi d'arrêter... On peut en parler, échanger des astuce et auss se soutenir.")
    ...
    if prompt := st.chat_input("Vous pouvez discuter avec Mr Doe"):
        # Display user message and add to history
        with st.chat_message("user"):
            st.write(prompt)
        ss.messages_smoker.append({"role": "user", "content": prompt})

        # Change thread session state key to be smoker-specific
